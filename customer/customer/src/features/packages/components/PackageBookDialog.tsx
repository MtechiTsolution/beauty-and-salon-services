...
import type { Branch, Package } from '@mit-salon/shared/types';
...
export function PackageBookDialog({ pkg, open, onOpenChange }: PackageBookDialogProps) {
  ...
  const { data: branches = [], isLoading: loadingBranches } = useQuery({
    queryKey: ['branches-package-book'],
    queryFn: () => branchesApi.list(),
    enabled: open,
  });

  const { data: services = [], isLoading: loadingServices } = useQuery({
    queryKey: ['services-package-book'],
    queryFn: () => servicesApi.list(),
    enabled: open,
  });

  const availableBranches = pkg ? getBranchesForPackage(pkg, branches, services) : [];
  const isLoading = loadingBranches || loadingServices;

  const handleSelectBranch = (branch: Branch) => {
    if (!pkg) return;
    onOpenChange(false);
    navigate(`/book?package=${encodeURIComponent(pkg.id)}&branch=${encodeURIComponent(branch.id)}`);
  };
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="customer-package-book-dialog mit-dialog-content max-h-[min(90vh,36rem)] w-[min(92vw,28rem)] flex-col overflow-hidden p-0">
        <DialogTitle className="sr-only">Choose salon for package</DialogTitle>
        <DialogDescription className="sr-only">
          Select a salon location where this package is available.
        </DialogDescription>
        ...
        <div className="border-b px-5 py-4">
          ...
          <h2 className="font-heading mt-1 text-lg font-semibold">{pkg?.name ?? 'Package'}</h2>
          {pkg && (
            <p className="mt-1 text-sm text-muted-foreground">
              ${pkg.price} · {pkg.total_sessions} session{pkg.total_sessions === 1 ? '' : 's'} · ~
              {packageDurationMinutes(pkg, services)} min visit
            </p>
          )}
        ...
        <div className="flex-1 overflow-y-auto px-3 py-3">
          {isLoading ? (
            ...
            <div className="rounded-xl border border-dashed bg-muted/30 px-4 py-10 text-center">
              ...
              <p className="mt-3 text-sm font-medium">No salons available for this package</p>
              <p className="mt-1 text-xs text-muted-foreground">
                The included services may not be offered at any location yet.
              </p>
            </div>
          ) : (
